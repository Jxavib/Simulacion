...
p <- 0.85
q <- 1-p
b1 <- dnorm(((55-n*p)/sqrt(n*p*q)),0,1)

b1 <- as.character(b1)
```
b) 
    $P(X=55)=P\left(\frac{X-np}{\sqrt{npq}}=\frac{55-68}{\sqrt{10.2}}\right)=$``r b1``

c)  
```{r include=F}
  c1 <- pnorm(((65-n*p)/sqrt(n*p*q)),0,1,lower.tail = F)

```
    $P(X\geq 65)=P\left(\frac{X-np}{\sqrt{npq}}\geq\frac{65-68}{\sqrt{10.2}}\right)=$``r c1``

d)
```{r include=F}
  r1=pnorm(c(((60-n*p)/sqrt(n*p*q)),((75-n*p)/sqrt(n*p*q))),0,1)
  r2=pnorm(c(60,75),n*p,sqrt(n*p*q))
  
d1=r1[2]-r1[1]
d2=r2[2]-r2[1]
```
    $P(65\leq X\leq 75)=P\left(\frac{65-68}{\sqrt{10.2}} \leq\frac{X-np}{\sqrt{npq}}\leq\frac{75-68}{\sqrt{10.2}}\right)=$``r d1``
    
e)
```{r include=F}
  e1=qnorm(0.35,0,1,lower.tail = F)
```
  ``r e1``
